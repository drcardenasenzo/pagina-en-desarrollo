(function () {
  ...
  const articlePendingImage = "/en/assets/img/placeholders/article-pending.svg";
  const articleImages = [
  ...
  const hydrateArticleImage = (img) => {
    ...
    if (!slug) return;
    img.dataset.articleImage = `/en/assets/img/articles/${slug}.png`;
  ...
  if (homeArticleGrid) {
    ...
    const step = 6;
    const shouldPaginate = cards.length > step;
    let visibleCount = shouldPaginate ? step : cards.length;
    if (articleControls) articleControls.hidden = !shouldPaginate;
    const updateArticleVisibility = () => {
      ...
      if (count)
        count.textContent = `Showing ${visibleCount} of ${cards.length} articles`;
    };
  ...
  if (archiveGrid) {
    ...
    const updateArchive = () => {
      ...
      if (count)
        count.textContent = `Showing ${Math.min(visibleCount, matches.length)} of ${matches.length} articles`;
    };
